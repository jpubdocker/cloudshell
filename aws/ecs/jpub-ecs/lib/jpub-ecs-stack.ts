...
export class JpubEcsStack extends cdk.Stack {
  constructor(scope: Construct, id: string, props?: cdk.StackProps) {
    ...
    // ECS TaskDefinition
    const echoTaskDefinition = new ecs.FargateTaskDefinition(
      this,
      "TaskDefinitionEcho",
      {
        cpu: 256,
    ...
    const nginxContainer = echoTaskDefinition.addContainer("EchoConNginx", {
      ...
      image: ecs.ContainerImage.fromRegistry(
        "ghcr.io/jpubdocker/simple-nginx-proxy:v0.0.1"
      ),
    ...
    const echoContainer = echoTaskDefinition.addContainer("EchoConEcho", {
      containerName: "echo",
      image: ecs.ContainerImage.fromRegistry(
        "ghcr.io/jpubdocker/echo:v0.0.1-9-gfe27471-slim"
      ),
      logging: ecs.LogDrivers.awsLogs({
        streamPrefix: "echo-nginx",
        logRetention: log.RetentionDays.ONE_MONTH,
